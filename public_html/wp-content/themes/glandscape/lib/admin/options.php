...
$gblu_getSlideTerms = array();
	foreach ($slideterms as $term_list) {
		$gblu_getSlideTerms [$term_list->term_id] = $term_list->name;
		//echo $term_list->name;
}
$gblu_fontsizes = array(
			'' 	 => 'Theme default size',	
			'10px' => '10 pixels',
			'11px' => '11 pixels',
			'12px' => '12 pixels',
...
$options = array (
	...
	array("type" => "break"),
		...
		array(
				...
				"name" => "Answer",
				"desc" => "",
				"id" => $shortname."_trans_answer",
				"type" => "text"),
				
				array(
				"name" => "Regards",
				"desc" => "",
				"id" => $shortname."_trans_regards",
				"type" => "text"),
...
if ( 'save' == $_REQUEST['action'] & 'options.php' == $_REQUEST['page'] ) {
		foreach ($options as $value) {
			...
			header("Location:?page=".$_REQUEST['page'] ."&saved=true");
				...
				if($_REQUEST['gblu_blog_ex_cat']!=""){
							$slider_category_final="";
							foreach($_REQUEST['gblu_blog_ex_cat']  as $slider_category) {
									$slider_category_final .= $slider_category .",";
							}
							update_option( "gblu_blog_ex_cat[]", $slider_category_final);
						}
				
		die;
}
function gblu_general(){	
	global $options;
	admin_head_addition();
	$ca=gblu_menu('1');
	echo $ca;
	gblu_generate_form($options);
