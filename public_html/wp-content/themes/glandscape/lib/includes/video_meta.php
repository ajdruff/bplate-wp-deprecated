		...
		*/
		function createCustomFields() {
			if ( function_exists( 'add_meta_box' ) ) {
				...
				add_meta_box( 'my-custom-fields', 'Video or Flash content', array( &$this, 'displayCustomFields' ), 'gbluportfolio', 'normal', 'high' );
				...
				foreach ( $this->videometa as $customField ) {
					...
					foreach ( $scope as $scopeItem ) {
						switch ( $scopeItem ) {
							...
							case "portfolio": {
								// Output on any post screen
								if ( basename( $_SERVER['SCRIPT_FILENAME'] )=="post-new.php" || $post->post_type=="gbluportfolio" )
									$output = true;
								break;
							}
						...
						<div style="float:none; clear:none; background:">
							...
							switch ( $customField[ 'type' ] ) {
								...
								case "select": {
									...
									foreach ($gblu_getpages[$page_list ->ID] as $selectvalue   ){
